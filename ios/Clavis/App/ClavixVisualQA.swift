...
private struct ClavixVisualQAToday: View {
    ...
    private var dimensionSnapshot: some View {
        VQASection(eyebrow: "", title: "Five-axis snapshot") {
            HStack(spacing: 1) {
                ForEach([("FIN", 86), ("NEWS", 62), ("MAC", 71), ("SEC", 78), ("VOL", 80)], id: \.0) { item in
                    VStack(spacing: 8) {
                        ...
                        VQADimensionPill(score: item.1, tone: dimensionTone(item.0))
                    }
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 14)
                    .background(Color.vqaPaper)
    ...
    private var attention: some View {
        VQASection(eyebrow: "", title: "Key Alerts") {
    ...
    private var bookPreview: some View {
        VQASection(eyebrow: "", title: "Your Positions") {
    ...
    private var calendar: some View {
        VQASection(eyebrow: "", title: "What to Watch") {
            VQACard(padding: 14) {
                Text("No immediate portfolio-level risk driver found today. Track AMD for data center revenue growth rate deceleration or guidance cuts and keep the rest of the book on watch for earnings, filings, or macro shocks.")
                    .font(ClavisTypography.vqaCaption)
                    .foregroundColor(.vqaInk2)
...
private struct ClavixVisualQADigest: View {
    var body: some View {
        VQAScreen(eyebrow: "Daily risk brief", title: "Morning Report", trailing: AnyView(HStack(spacing: 14) { Image(systemName: "slider.horizontal.3"); Image(systemName: "doc") }.foregroundColor(.vqaInk))) {
            ...
            VQARomanSection("V", "What to Watch", tag: "Catalysts") {
                VQACard(padding: 14) {
                    Text("No immediate portfolio-level risk driver found today. Track AMD for data center revenue growth rate deceleration or guidance cuts and keep the rest of the book on watch for earnings, filings, or macro shocks.")
                        .font(ClavisTypography.vqaCaption)
                        .foregroundColor(.vqaInk2)
...
private struct VQASectorCell: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(sector.symbol).font(ClavisTypography.mono(12, weight: .bold))
            Text(sector.name).font(ClavisTypography.vqaCaption).foregroundColor(.vqaInk2)
            HStack {
                Text(sector.change).font(ClavisTypography.mono(12, weight: .semibold)).foregroundColor(sector.tone)
                Spacer()
            }
        }
        .padding(10)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(Color.vqaPaper)
        .overlay(RoundedRectangle(cornerRadius: 2, style: .continuous).stroke(Color.vqaRule, lineWidth: 1))
...
private struct VQABookRow: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 10) {
            Text(holding.ticker).font(ClavisTypography.mono(13, weight: .bold)).foregroundColor(.vqaInk)
            Spacer()
            VQAGrade(holding.grade, size: 24)
            Text(holding.delta == 0 ? "-" : holding.delta > 0 ? "+\(holding.delta)" : "\(holding.delta)")
                .font(ClavisTypography.mono(11, weight: .semibold))
                .foregroundColor(holding.delta < 0 ? .vqaBad : holding.delta > 0 ? .vqaGood : .vqaInk3)
            Text(holding.today).font(ClavisTypography.mono(11, weight: .semibold)).foregroundColor(holding.today.hasPrefix("-") ? .vqaBad : .vqaGood)
        }
        .padding(.horizontal, 14)
        .padding(.vertical, 12)
    }
...
private struct VQADimensionPill: View {
    let score: Int
    let tone: Color
    ...
    var body: some View {
        ...
        .background(Color.vqaPaper)
        .clipShape(Capsule())
        .overlay(Capsule().stroke(tone.opacity(0.38), lineWidth: 1))
    }
    ...
    private func segment(index: Int) -> some View {
        ...
        return GeometryReader { geo in
            ZStack(alignment: .leading) {
                Rectangle().fill(tone.opacity(0.12))
                Rectangle()
...
private func dimensionTone(_ code: String) -> Color {
    switch code {
    case "FIN", "SEC":
        return .vqaGood
    case "NEWS", "VOL":
