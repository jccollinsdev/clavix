...
def test_build_volatility_inputs_price_derived_only():
    """Volatility inputs must not contain any IV fields.

    Polygon free tier returns 403 on every options-snapshot call; the dead IV
    path was removed. This invariant prevents it from being re-introduced.
    """
    ...
    assert "implied_vol_30d" not in result
    assert "iv_rank" not in result
    assert "iv_source" not in result
    assert result.get("realized_vol_30d") is not None
    assert result.get("vol_ratio") is not None
