...
def _limited_data_result(news_item: dict) -> dict:
    title = news_item.get("title", "Major event")
    evidence_quality = news_item.get("evidence_quality", "title_only")
    return {
        "analysis_text": f"{title} has the potential to shift the risk profile, but the depth of evidence is insufficient to determine the scale or direction of impact. The headline exists, but whether follow-through supports or reverses the initial risk rating remains unknown.",
        "impact_horizon": "near_term",
        "risk_direction": "neutral",
        "confidence": 0.3
        if evidence_quality in {"title_only", "headline_summary"}
        else 0.45,
        "scenario_summary": "Material headline detected — the durable risk impact depends on supporting detail that is not yet available.",
        "key_implications": [
            "Check whether subsequent reporting, filings, or market reaction validate or reverse the headline's risk implication."
        ],
...
"""
    try:
        ...
        return _limited_data_result(news_item)
...
If evidence quality is title_only or headline_summary, do not act as if you read a full article. Lower confidence and describe the rating as limited by available data.
